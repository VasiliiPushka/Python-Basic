...
def start(message: Message) -> None:
    bot.send_message(
        ...
        f"- /set_lang. Выбрать направление перевода [{DEFAULT_LANG}]. По умолчанию направление с русского языка на английский\n"
...
def set_lang(message: Message) -> None:
    ...
    if chosen_lang not in available_langs:
        bot.send_message(
            message.chat.id, f"Такого направления перевода нет. Попробуйте еще раз."
        )
        return
...
def lookup_command(message: Message) -> None:
    bot.send_message(
        message.chat.id,
        f"Введите текст для поиска.\n" f"Для смены направления напишите /set_lang",
    )
